...
class TemplateMainWindow(QMainWindow):
    ...
    def _connect_crawler_signal(self):
        """连接爬虫信号"""
        if hasattr(self, 'crawl_tab') and hasattr(self, 'manage_tab'):
            # 这里我们需要修改 crawl_tab 来发送数据完成的信号
            # 暂时使用信号总线
            signal_bus.crawl_finished.connect(self._on_crawl_finished)
    
    def _on_crawl_finished(self, data, message):
        """爬虫完成处理"""
        # 将数据传递给管理标签页
        if hasattr(self, 'manage_tab'):
            self.manage_tab.load_data(data)
        
        # 切换到管理标签页
        self.tab_widget.setCurrentIndex(1)  # 切换到成就管理标签页
